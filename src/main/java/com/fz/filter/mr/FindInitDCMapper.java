...
public class FindInitDCMapper extends Mapper<DoubleArrStrWritable,NullWritable,DoubleWritable, NullWritable> {
	...
	@Override
	public void map(DoubleArrStrWritable key, NullWritable value,Context context)throws InterruptedException,IOException{
		context.getCounter(FilterCounter.MAP_COUNTER).increment(1L);
		...
		for(FileStatus f:fss){
			...
			try {
				...
				while (reader.next(dkey, dvalue)) {// 循环读取文件
					...
					context.write(distance, NullWritable.get());
				}
			} catch (Exception e) {
				e.printStackTrace();
			} finally {

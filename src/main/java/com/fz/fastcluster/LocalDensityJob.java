...
/**
 * 求每个'点向量'的局部密度
 * 即求这个点(用户)在给定的距离内有多少个用户
 * 输入为 <key,value>--> <distance,<id_i,id_j>> <距离，<向量i编号，向量j编号>>
 * 
 * Mapper： 输出向量i编号，1 向量j编号,1 Reducer: 输出 向量i编号，局部密度
 * 有些向量是没有局部密度的，当某个向量距离其他点的距离全部都大于给定阈值dc时就会发生
 * 
 * feat:com.fz.fastcluster.mr.LocalDensityMapper
 * &	com.fz.fastcluster.mr.LocalDensityReducer
 * 
 */
public class LocalDensityJob extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		if (otherArgs.length != 5) {
			System.err.println("Usage: fz.fast_cluster.LocalDensityJob <in> <out> <dc> <method> <num_reducer>");
			System.exit(4);
		}
		conf.set("METHOD", otherArgs[3]);
		conf.setDouble("DC", Double.parseDouble(otherArgs[2]));
		Job job = Job.getInstance(conf, "Count the near neighours with a given dc(distance):" + otherArgs[2]);

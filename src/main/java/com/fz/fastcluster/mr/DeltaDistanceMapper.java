 ...
 * 
 */
public class DeltaDistanceMapper extends Mapper<DoubleWritable, IntPairWritable, IntWritable, DoublePairWritable> {
	...
	public void setup(Context context) throws IOException {
		densityPath = new Path(context.getConfiguration().get("DENSITYPATH"));
		...
		for (FileStatus f : fss) {
			...
			try {
				reader = new SequenceFile.Reader(conf, Reader.file(f.getPath()), Reader.bufferSize(4096),
						Reader.start(0));
				IntWritable dkey = (IntWritable) ReflectionUtils.newInstance(reader.getKeyClass(), conf);
				DoubleWritable dvalue = (DoubleWritable) ReflectionUtils.newInstance(reader.getValueClass(), conf);
	...
	//距离，双方ID
	@Override
	public void map(DoubleWritable key, IntPairWritable value, Context context)
		...
		if (vectorI == max_density_vector_id || vectorJ == max_density_vector_id) {// 最大局部密度，需寻找离该点最大的距离,应该在reducer中判断
			...
			context.write(vector_i, density_distance);
			log.info("vector_i:{},density:{},distance:{}",
					new Object[] { max_density_vector_id, max_density, key.get() });
			return;
		...
		if (densityI < densityJ) {// 输出 <I,大于I 的density的distance>
			...
			//1和2比较局部密度，谁密度小，将谁的id写入，并将其局部密度和距离写入
			context.write(vector_i, density_distance);

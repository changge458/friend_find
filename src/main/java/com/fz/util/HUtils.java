...
public class HUtils {
	...
	public static final String FILTER_GETATTRIBUTES = FILTER + "/" + "getattributes";// 属性提取
	public static final String FILTER_GETMAXMIN = FILTER + "/" + "getmaxmin";// 获得列最大最小值
	public static final String FILTER_NORMALIZATION = FILTER + "/" + "normalization";// 归一化
	public static final String FILTER_FINDINITDC = FILTER + "/" + "findinitdc";// 寻找dc阈值
	public static final String FILTER_PREPAREVECTORS = FILTER + "/" + "preparevectors";// 准备距离向量
	public static final int FILTER_PREPAREVECTORS_FILES = 4;// 由数据库到hdfs产生的文件数
	public static final String FILTER_CALDISTANCE = FILTER + "/" + "caldistance";// 计算向量之间的距离
	...
	// fast cluster
	public static final String LOCALDENSITYOUTPUT = "/user/centos/localdensity";
	public static final String DELTADISTANCEOUTPUT = "/user/centos/deltadistance";
	public static final String DELTADISTANCEBIN = "/user/centos/deltadistance.bin";// 局部密度最大向量id存储路径
	public static final String SORTOUTPUT = "/user/centos/sort";
	public static final String FIRSTCENTERPATH = "/user/centos/_center/iter_0/clustered/part-m-00000";
	public static final String FIRSTUNCLUSTEREDPATH = "/user/centos/_center/iter_0/unclustered";
	public static final String CENTERPATH = "/user/centos/_center";

	public static final String CENTERPATHPREFIX = "/user/centos/_center/iter_";
	...
	public static List<CurrentJobInfo> getJobs() throws IOException {
		...
		for (JobStatus js : jss) {
			if (js.getStartTime() > jobStartTime) {
				jsList.add(
						new CurrentJobInfo(getJobClient().getJob(js.getJobID()), js.getStartTime(), js.getRunState()));
			}
		}
	...
	public static void printJobStatus(JobStatus js) {
		System.out.println(new java.util.Date() + ":jobId:" + js.getJobID().toString() + ",map:" + js.getMapProgress()
				+ ",reduce:" + js.getReduceProgress() + ",finish:" + js.getRunState());
	}
	...
	/**
	 * 获得Path路径 如果url包含hdfs:// ，那么flag使用true，否则flag使用false
	 * 
	 * @param url
	 * @param flag
	 * @return
		...
		try {
			...
			while (fss.hasNext()) {
				...
				if (file.isFile() && file.toString().contains("part")) {
					// 使用这个才能下载成功
					fs.copyToLocalFile(false, file.getPath(),
							new Path(dst, "hdfs_" + (i++) + HUtils.DOWNLOAD_EXTENSION), true);
				}
			}
		} catch (Exception e) {
			ret.put("flag", "false");
	 ...
	 * @param min_2
	 */
	public static void updateMin(DoubleArrStrWritable key, DoubleArrStrWritable min_1, int column) {
	 ...
	 * @param max_2
	 */
	public static void updateMax(DoubleArrStrWritable key, DoubleArrStrWritable max_1, int column) {
		for (int i = 0; i < column; i++) {
	...
	public static double findInitDC(double percent, String path, long iNPUT_RECORDS2) {
		Path input = null;
		if (path == null) {
			input = new Path(HUtils.getHDFSPath(HUtils.FILTER_CALDISTANCE + "/part-r-00000"));
		} else {
		...
		try {
			reader = new SequenceFile.Reader(conf, Reader.file(input), Reader.bufferSize(4096), Reader.start(0));
			DoubleWritable dkey = (DoubleWritable) ReflectionUtils.newInstance(reader.getKeyClass(), conf);
			Writable dvalue = (Writable) ReflectionUtils.newInstance(reader.getValueClass(), conf);
	...
	public static double[] getCenterVector(String input, String output, int k) throws IOException {
		...
		try {
			writer = SequenceFile.createWriter(conf, Writer.file(path), Writer.keyClass(DoubleArrStrWritable.class),
					Writer.valueClass(value.getClass()));
			for (int i = 0; i < queue.length; i++) {
				key = new DoubleArrStrWritable(getInputI(queue[i].substring(1, queue[i].length() - 1), ","));
				value.set(i + 1);
	...
	public static void readSeq(String url, String localPath) {
		...
		try {
			...
			DoubleArrStrWritable dkey = (DoubleArrStrWritable) ReflectionUtils.newInstance(reader.getKeyClass(), conf);
			DoublePairWritable dvalue = (DoublePairWritable) ReflectionUtils.newInstance(reader.getValueClass(), conf);
		...
		try {
			...
			for (int i = start; i <= iter_i; i++) {
				...
				while (files.hasNext()) {
					...
					reader = new SequenceFile.Reader(conf, Reader.file(path), Reader.bufferSize(4096), Reader.start(0));
					DoubleArrStrWritable dkey = (DoubleArrStrWritable) ReflectionUtils.newInstance(reader.getKeyClass(),
							conf);
					IntWritable dvalue = (IntWritable) ReflectionUtils.newInstance(reader.getValueClass(), conf);
	...
	/**
	 * 检查用户是否符合规则 规则 ：reputation>15,upVotes>0,downVotes>0,views>0的用户
	 * 
	 * @param user
	 * @return
	 */
	private static boolean checkUser(Object user) {
		...
		if (ud.getReputation() <= 15)
			return false;
		if (ud.getUpVotes() <= 0)
			return false;
		if (ud.getDownVotes() <= 0)
			return false;
		if (ud.getViews() <= 0)
			return false;
		return true;
	...
	public static Map<Object, Object> readSeq(String input, int k) {
		...
		try {
			reader = new SequenceFile.Reader(conf, Reader.file(path), Reader.bufferSize(4096), Reader.start(0));
			Writable dkey = (Writable) ReflectionUtils.newInstance(reader.getKeyClass(), conf);
			Writable dvalue = (Writable) ReflectionUtils.newInstance(reader.getValueClass(), conf);
	 ...
	 * @return
	 */
	public static List<Integer> getCentIds(Map<Object, Object> firstK, String numReducerDensity,
			String numReducerDistance) {
	 ...
	 * 根据给定的users提取出来每个聚类中心，并把其写入hdfs key,value--> <IntWritable
	 * ,DoubleArrIntWritable> --> <id,<type,用户有效向量>> 同时把聚类中心写入本地文件
	 * 
	 * @param localfile
	 * @param users
	 * @param output
	...
	public static void writecenter2hdfs(List<UserData> users, String localfile, String output) throws IOException {
		...
		try {
			...
			for (UserData user : users) {
				bw.write("id:" + user.getId() + "\tvector:[" + HUtils.doubleArr2Str(HUtils.getDoubleArr(user)) + "]");
				bw.newLine();
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static void copy(String input, String output) {
		...
		try {
			...
			Utils.simpleLog("数据从" + input.toString() + "传输到" + out.toString() + (flag ? "成功" : "失败") + "!");
		} catch (Exception e) {
			e.printStackTrace();
			Utils.simpleLog("数据从" + input.toString() + "传输到" + out.toString() + "失败" + "!");
		}
	}

	/**
	 * 获得一个文件夹的信息 用于比较前后两次文件夹的内容是否一致
	 * 
	 * @param folder
	 * @param flag
	 *            true则floder字符串包含fs信息，否则不包含
	 * @return
	...
	public static String getFolderInfo(String folder, String flag) throws FileNotFoundException, IOException {
		...
		for (FileStatus file : files) {
			...
			buff.append(filename.substring(filename.length() - 1)).append(":").append(file.getLen()).append(",");
		}
	...
	private static Collection<? extends UserGroup> resolve(Path path) {
		...
		try {
			reader = new SequenceFile.Reader(conf, Reader.file(path), Reader.bufferSize(4096), Reader.start(0));
			IntWritable dkey = (IntWritable) ReflectionUtils.newInstance(reader.getKeyClass(), conf);
			DoubleArrIntWritable dvalue = (DoubleArrIntWritable) ReflectionUtils.newInstance(reader.getValueClass(),
					conf);
